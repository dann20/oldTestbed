...
def main_function():
    ...
    while True:
        ...
        shm1a = shared_memory.SharedMemory(name = 'not_normalized',create = True, size = raw_feature_np.nbytes)
        # raw_feature_np = shm1a.buf
        shm1a.buf = raw_feature_np
...
def normalization():
    print('normalization')
    shm1b = shared_memory.SharedMemory(name = 'not_normalized', create = False)
    print('normalization2')
    shm2a = shared_memory.SharedMemory(name = 'normalized', create = True, size = shm1b.buf.nbytes)
    while True:
        start = time.time()
        for y in range(shm1b.buf.shape[1]):
            for x in range(shm1b.buf.shape[0]):
                shm2a.buf[x,y] = (shm1b.buf[x,y]-shm1b.buf[:,y].min())/(shm1b.buf[:,y].max()-shm1b.buf[:,y].min())
        np.savetxt('normalized_features.csv',shm2a.buf,delimiter=',')   
        finish = time.time()
        print("Finished")
        if ((finish-start) < 30):
...
if __name__ == '__main__':
    with concurrent.futures.ProcessPoolExecutor() as executor:
        f1 = executor.submit(main_function)
        f2 = executor.submit(normalization)
    print('main')
